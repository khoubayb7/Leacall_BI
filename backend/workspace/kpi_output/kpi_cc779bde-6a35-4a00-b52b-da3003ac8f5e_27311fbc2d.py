...
SHARED_FIELD_ALIASES = {
    "lead_id": ["id", "lead_id", "uuid"],
    "attempts": ["attempts", "call_attempts", "total_attempts"],
    "connected": ["connected", "is_connected", "answered", "contacted"],
    "qualified": ["qualified", "is_qualified"],
    "converted": ["converted", "is_converted", "sale", "won", "closed_won"],
}
...
def _normalize_shared_record(row: Dict[str, Any]) -> Dict[str, Any]:
    lead_id = _first_value(row, SHARED_FIELD_ALIASES["lead_id"], default="")
    attempts = _to_number(_first_value(row, SHARED_FIELD_ALIASES["attempts"], default=0))
    connected = _to_bool(_first_value(row, SHARED_FIELD_ALIASES["connected"], default=False))
    qualified = _to_bool(_first_value(row, SHARED_FIELD_ALIASES["qualified"], default=False))
    converted = _to_bool(_first_value(row, SHARED_FIELD_ALIASES["converted"], default=False))
...
def _compute_shared_metrics(shared_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    leads_count = len(shared_rows)
    ...
    for row in shared_rows:
        ...
        connected_leads += 1 if row["connected"] else 0
        qualified_leads += 1 if row["qualified"] else 0
        converted_leads += 1 if row["converted"] else 0
...
def _compute_client_specific_metrics(_records: List[Dict[str, Any]]) -> Dict[str, Any]:
    return {}
...
def generate_kpis(dataset_file_path: str) -> Dict[str, Any]:
    ...
    shared_rows = [_normalize_shared_record(row) for row in records]
    shared_metrics = _compute_shared_metrics(shared_rows)
    client_specific_metrics = _compute_client_specific_metrics(records)
    ...
    return {
        ...
        "shared_fields_schema": list(SHARED_FIELD_ALIASES.keys()),
        "shared_metrics": shared_metrics,
        "client_specific_metrics": client_specific_metrics,
    }

if __name__ == "__main__":
    dataset_file_path = "C:\\Users\\khoub\\OneDrive\\Bureau\\BI_System\\backend\\workspace\\kpi_output\\datasets\\dataset_cc779bde-6a35-4a00-b52b-da3003ac8f5e_7e8dafb3.json"
